...
const A5TStaking = () => {
  ...
  const connect = async () => {
    ...
    if (res) {
      setState({
        ...state,
        address: eth.address,
        USDC_balance: eth.USDC_balance,
        A5T_balance: eth.A5T_balance,
        A5T_USDC_balance: eth.A5T_USDC_balance,
      });
      onAlert({
        type: "INFO",
        title: "Online",
  ...
  const tick = async () => {
    if (eth.connected && !loaded) {
      ...
      setState({
        ...state,
        start_date: await Staking.window_start_date(),
        end_date: await Staking.window_end_date(),
  ...
  const stake = (pool_number) => {
    if (state.end_date == 0 || state.start_date == 0) {
  ...
  const onStake = async (pool_number, amount) => {
    hideModal();
    if (parseFloat(state.A5T_USDC_balance) < parseFloat(amount)) {
  ...
  const isPoolComplete = (duration) => {
    ...
    if (
      parseInt(state.end_date) + parseInt(duration) < timeStamp &&
      parseInt(state.end_date) > 0
    ) {
      return true;
    }
    return false;
  ...
  console.log("pool_count", poolbundle);
  const openLink = (addr) => {
              ...
              <CardContent rowNo={4} colNo={1}>
                ...
                <CardFields>
                  ...
                  <CardFieldValue>
                    ...
                    {state?.start_date == 0
                      ? "Not Set"
                      : utils.convertTimeStamp(state?.start_date * 1000)}
                  </CardFieldValue>
                </CardFields>
                <CardFields>
                  <CardFieldName>
                  ...
                  <CardFieldValue>
                    {state?.end_date == 0
                      ? "Not Set"
                      : utils.convertTimeStamp(state?.end_date * 1000)}
                  </CardFieldValue>
                </CardFields>
                <CardFields>
                  <CardFieldName>Total Pools:</CardFieldName>
                  <CardFieldValue>{state?.pool_count}</CardFieldValue>
                </CardFields>
              </CardContent>
